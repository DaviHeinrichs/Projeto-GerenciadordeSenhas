...
from utils.telas.ui_Confirmacao_window import Ui_confirmar_window

class confirmar2(QDialog):
    def __init__(self,*args,**argvs):
        super(confirmar2,self).__init__(*args,**argvs)
        self.ui = Ui_confirmar_window()
        self.ui.setupUi(self)
        self.ui.Enviar.clicked.connect(self.promover)
        self.ui.DeleteUser.clicked.connect(self.voltar)
        
    def promover(self):
        from utils.erro_comum import mostrar_informacao, mostrar_erro
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../bin')))
        from interface.users.user_data.statements.main import promover_admin
        user_id = self.ui.EmailInput.text()
        if user_id.strip():
            if promover_admin(user_id)==True:
                mostrar_informacao("Usuário Promovido!", "Reinicie o programa!")
            elif promover_admin(user_id)==False:
                mostrar_erro("Digite um ID válido!", "User_id não encontrado!")
        if not user_id.strip():
            mostrar_erro("Digite um ID!", "Você não digitou um ID!")
        
    def voltar(self):
        self.close()

class confirmar(QDialog):
    def __init__(self,*args,**argvs):
        super(confirmar,self).__init__(*args,**argvs)
        self.ui = Ui_confirmar_window()
        self.ui.setupUi(self)
        self.ui.Enviar.clicked.connect(self.deletar)
        self.ui.DeleteUser.clicked.connect(self.voltar)
    
    def deletar(self):
        from utils.erro_comum import mostrar_informacao, mostrar_erro
        sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../../../bin')))
        from interface.users.user_data.statements.main import deletar_usuário
        user_id = self.ui.EmailInput.text()
        
        if user_id.strip():
            if deletar_usuário(user_id)==True:
                mostrar_informacao("Usuário Deletado!", "Reinicie o programa!")
            elif deletar_usuário(user_id)==False:
                mostrar_erro("Digite um ID válido!", "User_id não encontrado!")
        if not user_id.strip():
            mostrar_erro("Digite um ID!", "Você não digitou um ID!")
...
class tela_admin(QDialog):
    def __init__(self,*args,**argvs):
        ...
        self.ui.DeleteUser.clicked.connect(self.deletar)
        self.ui.UserToAdmin.clicked.connect(self.promover)
